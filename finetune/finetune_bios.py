...
from bios_llm.datasets import DATASET
from bios_llm.utils.logging import setup_basic_logger
...
class GlobalStepLogging(TrainerCallback):
...
def get_args():
    ...
    parser.add_argument("--load_best_model_at_end", type=strtobool, default=1)
    parser.add_argument("--optim", type=str, default="adamw_torch")
    parser.add_argument("--run_name", type=str, default="StarCoder-finetuned", help='wandb run name')
    parser.add_argument("--naive_chunks_path", type=str, default="data.pt",
                        help='Path of output file using `convert_chunks_into_ids.py`')
...
def run_training(args, train_data, val_data):
    ...
    LORA_TARGET_MODULES = {
        'starcoder-3': ["c_proj", "c_attn", "q_attn"],
        'starcoder-5': [
            "c_proj",
            "c_attn",
            "q_attn",
    ...
    trainer = Trainer(model=model, args=training_args, train_dataset=train_data, eval_dataset=val_data,
                      callbacks=[SavePeftModelCallback, LoadBestPeftModelCallback, GlobalStepLogging])
...
def main(args):
    ...
    create_dataset_fn: Callable[..., tuple] = DATASET.get(args.dataset_name, default=create_datasets)
    train_dataset, eval_dataset = create_dataset_fn(
        args=args,
        tokenizer=tokenizer,
    )
